...
describe("main handler processing automatic releases", () => {
  ...
  beforeEach(() => {
    ...
    process.env["INPUT_VERBOSE"] = testInputVerbose.toString();
  ...
  it("should create a new release tag", async () => {
    ...
    const compareCommitsPayload = JSON.parse(
      fs.readFileSync(
        path.join(__dirname, "payloads", "compare-commits.json"),
        "utf8"
      )
    );
    ...
    const getCommitsSinceRelease = nock("https://api.github.com")
      ...
      .get(
        `/repos/marvinpinto/private-actions-tester/compare/HEAD...${testGhSHA}`
      )
    ...
    const getRef = nock("https://api.github.com")
      .matchHeader("authorization", `token ${testGhToken}`)
      .get(
        `/repos/marvinpinto/private-actions-tester/git/refs/tags/${testInputAutomaticReleaseTag}`
      )
      .reply(404);
    ...
    const listAssociatedPRs = nock("https://api.github.com")
      .matchHeader("authorization", `token ${testGhToken}`)
      .get(
        `/repos/marvinpinto/private-actions-tester/commits/${testGhSHA}/pulls`
      )
      .reply(200, []);
    ...
    const getReleaseByTag = nock("https://api.github.com")
      .matchHeader("authorization", `token ${testGhToken}`)
      .get(
        `/repos/marvinpinto/private-actions-tester/releases/tags/${testInputAutomaticReleaseTag}`
      )
      .reply(400);
  ...
  it("should update an existing release tag", async () => {
    ...
    const getCommitsSinceRelease = nock("https://api.github.com")
      .matchHeader("authorization", `token ${testGhToken}`)
      .get(
        `/repos/marvinpinto/private-actions-tester/compare/${testInputAutomaticReleaseTag}...${testGhSHA}`
      )
      .reply(200, compareCommitsPayload);
